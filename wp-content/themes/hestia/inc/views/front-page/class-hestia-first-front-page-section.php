...
class Hestia_First_Front_Page_Section extends Hestia_Abstract_Main
{
     ...
     * @since 1.1.72
     */
    public function maybe_render_slider()
    {
        echo '<div id="smart-slider">';
        $slider3Id = get_theme_mod('hestia_east_slider', apply_filters('hestia_east_slider_default', false));
        if ($slider3Id != '') {
            echo do_shortcode('[smartslider3 slider=' . $slider3Id . ']');
        }
        echo '</div>';
    }
     ...
     * Utility to check if we should display parallax.
     */
    public static function should_display_slider()
    {
        $hestia_big_title_type = get_theme_mod('hestia_slider_type', apply_filters('hestia_slider_type_default', 'image'));
        if (empty($hestia_big_title_type) || $hestia_big_title_type !== 'slider') {
            return false;
        }
        $slider3Id = get_theme_mod('hestia_east_slider', apply_filters('hestia_east_slider_default', false));
        if (empty($slider3Id)) {
